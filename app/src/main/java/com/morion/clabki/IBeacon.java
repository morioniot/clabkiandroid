...
public class IBeacon extends Beacon {
    ...
    final private static int UUID_SIZE = 16;
    final private static int UUID_INDEX = 9;
    final private static int MAJOR_INDEX = 25;
    final private static int MINOR_INDEX = 27;
    final private static int RSSI_AT_1M_INDEX = 29;
    ...
    public IBeacon(BluetoothDevice beacon) {
        super(beacon);
        this.protocol = Beacon.IBEACON_PROTOCOL;
    }
    ...
    public static String extractUUID(byte[] advertisedPackage, int[] chunkSizes) throws Exception {
        if(IBeacon.isValidUUIDChunksArray(chunkSizes)) {
            final HexUtil hexTool = new HexUtil();
            final String[] packageInHexStrings = hexTool.byteArrayToHexStrArray(advertisedPackage);
            final StringBuilder stringBuilder = new StringBuilder();
    ...
    public static int extractMajor(byte[] advertisedPackage) {
        return IBeacon.extract2BytesId(advertisedPackage, MAJOR_INDEX);
    }

    public static int extractMinor(byte[] advertisedPackage) {
        return IBeacon.extract2BytesId(advertisedPackage, MINOR_INDEX);
    }
